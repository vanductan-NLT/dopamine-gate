"use strict";(()=>{var m={BLOCKLIST:"blocklist",API_KEY:"apiKey",LOGS:"logs",IS_ENABLED:"isEnabled"};async function u(e){return new Promise(i=>{chrome.storage.sync.get(e,t=>{i(t)})})}async function k(e){return new Promise(i=>{chrome.storage.sync.set(e,i)})}async function c(){return(await u([m.API_KEY])).apiKey??""}var I=100;async function p(e){let i=await A();i.unshift(e);let t=i.slice(0,I);await k({logs:t})}async function A(){return(await u([m.LOGS])).logs??[]}function v(e){return e.futureFeeling==="Waste"?{decision:"block",confidence:1,message:"You already identified this as a waste of time. Let's stop here! \u{1F6D1}"}:e.outcome==="Emptiness"?{decision:"block",confidence:.9,message:"Don't let yourself feel empty after scrolling. Go do something meaningful! \u2728"}:null}var l=20;var s=1,d=6;function b(){return document.getElementById("dopamine-gate-overlay")!==null}function h(){return window.location.hostname}async function w(){if(console.log("[Dopamine Gate] Content Script v2.1 - Initializing..."),!window.dg_initializing){window.dg_initializing=!0;try{if(b()){console.log("[Dopamine Gate] Overlay already exists, skipping");return}await c()?T():S(),window.dg_observer_active||(x(),window.dg_observer_active=!0)}finally{window.dg_initializing=!1}}}function x(){new MutationObserver(i=>{if(!b()){console.log("[Dopamine Gate] Blocker was removed by site. Re-injecting..."),w();return}document.body.style.overflow!=="hidden"&&(document.body.style.overflow="hidden")}).observe(document.documentElement,{childList:!0,subtree:!0})}function T(){let e=document.createElement("div");e.id="dopamine-gate-overlay",e.className="dopamine-gate-overlay",e.innerHTML=`
  ...
  `,document.body.appendChild(e),document.body.style.overflow="hidden",D()}function S(){var i;let e=document.createElement("div");e.id="dopamine-gate-overlay",e.className="dopamine-gate-overlay",e.innerHTML=`
  ...
  `,document.body.appendChild(e),document.body.style.overflow="hidden",(i=document.getElementById("dg-btn-close-warning"))==null||i.addEventListener("click",()=>{chrome.runtime.sendMessage({type:"CLOSE_TAB"})})}function D(){let e=document.getElementById("dopamine-gate-reflection-form"),i=document.getElementById("dg-btn-leave"),t=document.getElementById("dg-btn-next"),n=document.getElementById("dg-btn-back");g(document.getElementById("dg-reason"),"dg-reason-counter"),g(document.getElementById("dg-goal-target"),"dg-goal-counter"),g(document.getElementById("dg-alternative"),"dg-alternative-counter"),i==null||i.addEventListener("click",()=>{chrome.runtime.sendMessage({type:"CLOSE_TAB"})}),t==null||t.addEventListener("click",()=>{C()&&f(s+1)}),n==null||n.addEventListener("click",()=>{f(s-1)}),e==null||e.addEventListener("submit",P)}function f(e){if(e<1||e>d)return;document.querySelectorAll(".dopamine-gate-step").forEach(n=>n.classList.remove("active"));let t=document.querySelector(`.dopamine-gate-step[data-step="${e}"]`);t==null||t.classList.add("active"),s=e,B()}function B(){let e=document.getElementById("dg-btn-next"),i=document.getElementById("dg-btn-submit"),t=document.getElementById("dg-btn-back"),n=document.getElementById("dg-progress-bar");!e||!i||!t||!n||(s===d?(e.style.display="none",i.style.display="inline-flex"):(e.style.display="inline-flex",i.style.display="none"),t.style.display=s>1?"inline-flex":"none",n.style.width=`${s/d*100}%`)}function C(){let e=document.querySelector(`.dopamine-gate-step[data-step="${s}"]`),i=e==null?void 0:e.querySelectorAll("textarea, select, input[required]"),t=!0;return i.forEach(n=>{if(n.tagName==="TEXTAREA")n.value.length<l?(n.classList.add("invalid"),t=!1):n.classList.remove("invalid");else if(n.tagName==="SELECT")n.value?n.classList.remove("invalid"):(n.classList.add("invalid"),t=!1);else if(n.type==="radio"){let o=n.getAttribute("name");document.querySelector(`input[name="${o}"]:checked`)||(t=!1)}}),t}function g(e,i){let t=document.getElementById(i);if(!e||!t)return;let n=()=>{let o=(e.value||"").length;t&&(t.textContent=`${o}/${l}`,t.classList.remove("warning","valid"),o>=l?t.classList.add("valid"):o>0&&t.classList.add("warning"))};["input","keyup","change","paste"].forEach(o=>{e.addEventListener(o,n)}),n()}async function P(e){e.preventDefault();let i=e.target,t=new FormData(i),n={reason:t.get("reason")||"",goalTarget:t.get("goalTarget")||"",alternativeAction:t.get("alternativeAction")||"",outcome:t.get("outcome")||"Emptiness",needType:t.get("needType")||"Dopamine",futureFeeling:t.get("futureFeeling")||"Waste"};if(O(n)){_();try{let o=v(n);if(!o){let a=chrome.runtime.sendMessage({type:"EVALUATE_REFLECTION",answers:n}),L=new Promise((G,E)=>setTimeout(()=>E(new Error("AI Analysis timeout (30s)")),3e4)),r=await Promise.race([a,L]);if(r&&r.success)o=r.decision;else throw new Error((r==null?void 0:r.error)||"Background evaluation failed")}if(o){let a={timestamp:Date.now(),domain:h(),answers:n,aiDecision:o};await p(a),y(o)}}catch(o){console.error("[Dopamine Gate] Evaluation error:",o),y({decision:"block",confidence:1,message:"AI Connection failure. Defaulting to block for safety."})}}}function O(e){let i=!0,t=document.getElementById("dg-reason-error"),n=document.getElementById("dg-reason");e.reason.length<l?(t==null||t.classList.add("visible"),n==null||n.classList.add("invalid"),i=!1):(t==null||t.classList.remove("visible"),n==null||n.classList.remove("invalid"));let o=document.getElementById("dg-alternative-error"),a=document.getElementById("dg-alternative");return e.alternativeAction.length<l?(o==null||o.classList.add("visible"),a==null||a.classList.add("invalid"),i=!1):(o==null||o.classList.remove("visible"),a==null||a.classList.remove("invalid")),i}function _(){let e=document.getElementById("dopamine-gate-form");e&&(e.innerHTML=`
  ...
  `)}function y(e){var a;let i=document.getElementById("dopamine-gate-form");if(!i)return;let t=e.decision==="block",n=t?"Access Blocked":"Access Allowed",o=t?"blocked":"allowed";i.innerHTML=`
  ...
  `,t?M():(a=document.getElementById("dg-btn-proceed"))==null||a.addEventListener("click",()=>{R()})}function M(){let e=5,i=document.getElementById("dg-countdown"),t=setInterval(()=>{e--,i&&(i.textContent=e.toString()),e<=0&&(clearInterval(t),chrome.runtime.sendMessage({type:"CLOSE_TAB"}))},1e3)}function R(){let e=document.getElementById("dopamine-gate-overlay");e==null||e.remove(),document.body.style.overflow=""}w();})();

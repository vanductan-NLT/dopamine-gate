"use strict";(()=>{var g={BLOCKLIST:"blocklist",API_KEY:"apiKey",LOGS:"logs"};async function m(e){return new Promise(i=>{chrome.storage.sync.get(e,t=>{i(t)})})}async function w(e){return new Promise(i=>{chrome.storage.sync.set(e,i)})}async function d(){return(await m([g.API_KEY])).apiKey??""}var L=100;async function u(e){let i=await E();i.unshift(e);let t=i.slice(0,L);await w({logs:t})}async function E(){return(await m([g.LOGS])).logs??[]}function p(e){return e.futureFeeling==="Waste"?{decision:"block",confidence:1,message:"You already identified this as a waste of time. Let's stop here! \u{1F6D1}"}:e.outcome==="Emptiness"?{decision:"block",confidence:.9,message:"Don't let yourself feel empty after scrolling. Go do something meaningful! \u2728"}:null}var l=20;var s=1,r=6;function y(){return document.getElementById("dopamine-gate-overlay")!==null}function b(){return window.location.hostname}async function h(){if(console.log("[Dopamine Gate] Content Script v2.1 - Initializing..."),!window.dg_initializing){window.dg_initializing=!0;try{if(y()){console.log("[Dopamine Gate] Overlay already exists, skipping");return}await d()?I():A(),window.dg_observer_active||(k(),window.dg_observer_active=!0)}finally{window.dg_initializing=!1}}}function k(){new MutationObserver(i=>{if(!y()){console.log("[Dopamine Gate] Blocker was removed by site. Re-injecting..."),h();return}document.body.style.overflow!=="hidden"&&(document.body.style.overflow="hidden")}).observe(document.documentElement,{childList:!0,subtree:!0})}function I(){let e=document.createElement("div");e.id="dopamine-gate-overlay",e.className="dopamine-gate-overlay",e.innerHTML=`
  ...
  `,document.body.appendChild(e),document.body.style.overflow="hidden",x()}function A(){var i;let e=document.createElement("div");e.id="dopamine-gate-overlay",e.className="dopamine-gate-overlay",e.innerHTML=`
  ...
  `)}function f(e){var a;let i=document.getElementById("dopamine-gate-form");if(!i)return;let t=e.decision==="block",n=t?"Access Blocked":"Access Allowed",o=t?"blocked":"allowed";i.innerHTML=`
  ...
  `,t?P():(a=document.getElementById("dg-btn-proceed"))==null||a.addEventListener("click",()=>{O()})}function P(){let e=5,i=document.getElementById("dg-countdown"),t=setInterval(()=>{e--,i&&(i.textContent=e.toString()),e<=0&&(clearInterval(t),chrome.runtime.sendMessage({type:"CLOSE_TAB"}))},1e3)}function O(){let e=document.getElementById("dopamine-gate-overlay");e==null||e.remove(),document.body.style.overflow=""}h();})();
